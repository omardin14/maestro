...
from maestro.planner.llm import get_llm
from maestro.planner.state import (
    DOD_ITEMS,
...
from maestro.planner.prompts.analyzer import get_analyzer_prompt
from maestro.planner.prompts.epic_generator import get_epic_generator_prompt
...
from maestro.planner.prompts.sprint_planner import get_sprint_planner_prompt
from maestro.planner.prompts.story_writer import MAX_STORIES_PER_EPIC, MIN_STORIES_PER_EPIC, get_story_writer_prompt
from maestro.planner.prompts.system import get_system_prompt  # noqa: E402 — direct submodule imports avoid circular import
from maestro.planner.prompts.task_decomposer import get_task_decomposer_prompt
from maestro.planner.tools import detect_platform
...
def _batch_defaults_for_phase(questionnaire: QuestionnaireState) -> tuple[list[str], int]:
    ...
    """
    from maestro.planner.state import PHASE_QUESTION_RANGES
...
def _is_jira_configured() -> bool:
    ...
    """
    from maestro.config import get_jira_base_url, get_jira_email, get_jira_project_key, get_jira_token
...
def _fetch_jira_velocity() -> dict | None:
    ...
    """
    try:
        from maestro.planner.tools.jira import jira_fetch_velocity
...
def _fetch_active_sprint_number() -> tuple[int | None, str | None, str]:
    ...
    """
    try:
        from maestro.planner.tools.jira import jira_fetch_active_sprint
...
def _load_user_context(path: str | None = None, docs_dir: str | None = None) -> tuple[str | None, dict]:
    ...
    """
    try:
        from maestro.planner.tools.codebase import load_project_context
...
def _fetch_confluence_context(questionnaire: QuestionnaireState) -> tuple[str | None, dict]:
    ...
    """
    try:
        from maestro.config import get_jira_base_url, get_jira_email, get_jira_token
        ...
        from maestro.planner.tools.confluence import confluence_search_docs
...
def _scan_repo_context(questionnaire: QuestionnaireState) -> tuple[str | None, dict]:
    ...
    try:
        if platform == "GitHub":
            from maestro.planner.tools.github import github_read_readme, github_read_repo
            ...
            from maestro.planner.tools.azure_devops import azdevops_read_repo
            ...
            from maestro.planner.tools.codebase import read_codebase
...
def _detect_bank_holidays_for_window(
    ...
    """
    from maestro.planner.tools.calendar_tools import _detect_country_from_locale, get_bank_holidays_structured
...
def _prepare_bank_holiday_choices(questionnaire: QuestionnaireState) -> None:
    ...
    Args:
        questionnaire: The mutable QuestionnaireState to update.
    """
    from maestro.planner.tools.calendar_tools import detect_bank_holidays
    ...
    from maestro.planner.tools.calendar_tools import get_bank_holidays_structured

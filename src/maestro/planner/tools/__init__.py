...
def get_tools() -> list[BaseTool]:
    ...
    resolve). Lazy import inside the function means `from maestro.planner.tools import
    get_tools` always succeeds, and the ImportError surfaces only when get_tools()
    is called.
    # See README: "Tools" — tool types, @tool decorator
    """
    from maestro.planner.tools.azure_devops import (
        azdevops_list_work_items,
        azdevops_read_file,
        azdevops_read_repo,
    )
    from maestro.planner.tools.calendar_tools import detect_bank_holidays
    from maestro.planner.tools.codebase import load_project_context, read_codebase, read_local_file
    from maestro.planner.tools.confluence import (
    ...
    from maestro.planner.tools.github import (
        github_list_issues,
        github_read_file,
        github_read_readme,
        github_read_repo,
    )
    from maestro.planner.tools.jira import (
        jira_create_epic,
        jira_create_sprint,
        jira_create_story,
    ...
    from maestro.planner.tools.llm_tools import estimate_complexity, generate_acceptance_criteria

...
from maestro.planner.tools.calendar_tools import _detect_country_from_locale, detect_bank_holidays
...
class TestDetectCountryFromLocale:
    ...
    def test_no_underscore_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.platform.system", lambda: "Linux")
    ...
    def test_macos_apple_locale_fallback(self, monkeypatch):
        ...
        monkeypatch.setattr("locale.getlocale", lambda: ("C", None))
        monkeypatch.setenv("LANG", "C")
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.platform.system", lambda: "Darwin")

        fake_result = subprocess.CompletedProcess(args=[], returncode=0, stdout="en_GB\n", stderr="")
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.subprocess.run", lambda *a, **kw: fake_result)
        assert _detect_country_from_locale() == "GB"
...
class TestDetectBankHolidays:
    ...
    def test_auto_detect_from_locale(self, monkeypatch):
        ...
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.locale.getlocale", lambda: ("en_GB", "UTF-8"))
        result = detect_bank_holidays.invoke(
            {
                "sprint_length_weeks": 2,
                "num_sprints": 6,
    ...
    def test_auto_detect_fails_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.locale.getlocale", lambda: ("C", None))
        monkeypatch.setenv("LANG", "C")
        monkeypatch.setattr("maestro.planner.tools.calendar_tools.platform.system", lambda: "Linux")
        result = detect_bank_holidays.invoke({})
        assert "Error" in result
...
class TestDetectBankHolidaysRegistered:
    ...
    def test_in_get_tools(self):
        ...
        from maestro.planner.tools import get_tools
